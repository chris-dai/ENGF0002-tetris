...
class Chris: # auto player class
    ...
    def choose_best_move(self,board):
        '''choose the best move comparing the score - we try 40 possibilities'''
        max_score = -1000000
        try:
            for rotation in range(4): # 4 possible rotations
                for pos in range(-5,5): # -5 as block drops from the middle
                    score_current, new_board = self.best_score(board, rotation, pos)
                    lookahead_score = -1000000
                    for rotation2 in range(4):  # lookahead
    ...
    def column_height(self,board):
        column_height = []  # create an empty list as there are 10 columns
        for x in range(board.width):
            for y in range(board.height):
                if (x,y) in board.cells:
                    column_height.append(board.height-y)  
                    break
                if y == board.height-1:
    ...
    def bumpiness(self, board):
        bump = 0 # bump is the diff between adjacent columns
    ...
    def complete_lines(self,diff):
        ''' 100 is the score by clearing one line, 25 is the score generated by moving down'''
        if 100 < diff < 125:
            result = 1
        elif 400 < diff < 425:
            result = 2
